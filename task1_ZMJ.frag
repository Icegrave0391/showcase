                      ...
                      out float t, out vec3 hitPos, out vec3 hitNormal ) 
{
	...
	vec3 centerPosition = sph.center;
	Ray_t rayN = ray;
	rayN.o = ray.o - centerPosition;
	//rayN.d = ray.d - centerPosition;
	
	float a = dot(rayN.d, rayN.d);
	float b = 2.0 * dot(rayN.d, rayN.o);
	float c = dot(rayN.o, rayN.o) - sph.radius * sph.radius;

	float delt = b * b - 4.0 * a * c;
	float t1, t2;

	if(delt > 0.0){
		t1 = (-b + sqrt(delt))/2.0 * a;
		t2 = (-b - sqrt(delt))/2.0 * a;
	...
	hitPos = rayN.o + t * rayN.d + centerPosition;
              ...
              out bool hasHit, out vec3 hitPos, out vec3 hitNormal, out vec3 k_rg ) 
{
    ...
    //   nearest hit point and the light source.
		...
		for(int j = 0; j < NUM_PLANES; j++){
			if(IntersectPlane( Plane[i], shadowRay, DEFAULT_TMIN, length(Light[i].position - nearest_hitPos))){
				inShadow[i] = true;
			}
			else{
				inShadow[i] = false;
		...
		for(int j = 0; j < NUM_SPHERES; j++){
			if(IntersectSphere( Sphere[i], shadowRay, DEFAULT_TMIN, length(Light[i].position - nearest_hitPos))){
				inShadow[i] = true;
			}
			else{
				inShadow[i] = false;
	...
	// * Then, call PhongLighting() to compute lighting for this light source.
	for(int i = 0; i < NUM_LIGHTS; i++) {
		I_local += PhongLighting( shadowRay.d, nearest_hitNormal, normalize(-ray.d), inShadow[i], 
